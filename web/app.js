...
/* =========================================================
   CINEVA FIREBASE HOME
   Admin movies + series only
   ========================================================= */
(function () {
    "use strict";

    function renderCinevaHome() {
        ...
        const movies = cinevaMovies.filter(function (m) {
            return m && (m.title || m.posterUrl || m.videoUrl);
        });

        const series = cinevaSeries.filter(function (s) {
            return s && (s.title || s.posterUrl);
        });
        ...
        if (!all.length) {
            home.innerHTML = `
                <div class="cineva-home-empty">
                    <h2>No movies or series yet</h2>
                    <p>Add content from the Admin Panel.</p>
                </div>
            `;
            return;
        }
        ...
        home.innerHTML = `
            <div class="cineva-home-hero"
                ${heroImage ? `style="background-image:linear-gradient(90deg,rgba(0,0,0,.88),rgba(0,0,0,.45),rgba(0,0,0,.12)),url('${esc(heroImage)}')"` : ""}>
                
                <div class="cineva-home-hero-content">
                    <span class="badge">
                        ${featured.type === "series" ? "SERIES" : "MOVIE"}
                    </span>

                    <h1>${esc(featured.title || "Untitled")}</h1>

                    <p>${esc(
                        featured.description ||
                        "Watch this on Cineva."
                    )}</p>

                    <button class="primary" id="cinevaHomeFeaturedButton">
                        ▶ Watch
                    </button>
                </div>
            </div>

            <section class="cineva-home-section">
                <h2>Movies</h2>
                <div class="cineva-home-grid" id="cinevaHomeMovies"></div>
            </section>

            <section class="cineva-home-section">
                <h2>Series</h2>
                <div class="cineva-home-grid" id="cinevaHomeSeries"></div>
            </section>
        `;

        const movieGrid = document.getElementById("cinevaHomeMovies");
        const seriesGrid = document.getElementById("cinevaHomeSeries");

        movies.forEach(function (movie) {
            movieGrid.appendChild(movieCard(movie));
        });

        series.forEach(function (series) {
            seriesGrid.appendChild(seriesCard(series));
        });

        const featuredButton =
            document.getElementById("cinevaHomeFeaturedButton");

        if (featuredButton) {
            featuredButton.onclick = function () {
                if (featured.type === "series") {
                    if (typeof showFirebaseSeries === "function") {
                        showFirebaseSeries(featured);
                    } else {
                        window.showPage("series");
                    }
                } else if (
                    typeof showFirebaseMovie === "function"
                ) {
                    showFirebaseMovie(featured);
                }
            };
        }
    }

    window.renderCinevaHome = renderCinevaHome;
})();
